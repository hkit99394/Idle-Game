...
Slices 2.9.1 and 2.9.2 are complete. Tactic comparison legacy damage fields and CSV columns stay for one more review cycle; `TACTIC_COMPARISON_EXPORT_SCHEMA_VERSION` remains `4`. Engine-level `outerDamage` / `innerDamage` payload keys also stay as stable internal replay/accounting fields for now. Stage 2.9 should stay conservative: do not remove compatibility adapters, temporary report/export columns, or stable engine payload fields until downstream comparison and replay needs have been checked and the affected schema or event contract has an explicit migration path.
