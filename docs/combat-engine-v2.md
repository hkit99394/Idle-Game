...
- `resolveStageBattle(staticData, input)` in `core/progression/battleResolution.ts` is the progression adapter. It uses an explicit `input.tacticId` when provided, otherwise reads `PlayerProgress.selectedRoutineId` and falls back to `balanced_routine` for missing or invalid saved values.
