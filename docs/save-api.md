...
Simulator, support-decision, and future backend tooling should use those current runtime progress/save fields whenever they read save-shaped data. Generated balance authoring reports remain separate from save JSON: static stage rewards and budget columns such as `reward_silver`, `reward_cultivation`, `reward_herbs`, and `reward_combat_experience` are content-report metrics, not persisted save fields. Tactic comparison exports are also report contracts rather than save contracts; schema version `4` exposes current `playerKineticDamage` / `playerCognitiveDamage` JSON fields and `player_kinetic_damage` / `player_cognitive_damage` CSV columns. Stage 2.9.1 keeps temporary legacy damage-channel aliases for one more review cycle; removing them later requires a tactic comparison export schema bump.

Stage 3.3 keeps District Heat report-only and Stage 3.4 is active for a non-punitive warning contract before any player-facing heat behavior. The simulator may expose `districtHeatProjection` and `districtHeatPromotionDecision` in full debug JSON, but current `SaveData`, browser storage, import/export save JSON, and cloud `rawSave` payloads must not persist `districtHeat`, `districtHeatProjection`, `districtHeatPromotionDecision`, `projectedHeat`, `heatBand`, `districtAttention`, warning copy, warning acknowledgement, or any related live heat state. Slice 100.3 selects a no-persistence warning posture: Stage 3.4 does not bump `SAVE_DATA_VERSION` beyond `13`, does not add a migration, does not add a browser storage key, and does not add save diagnostics output for the warning. A future persisted District Heat implementation must open a dedicated save-version slice before adding durable heat fields.
