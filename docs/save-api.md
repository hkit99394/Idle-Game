...
Stage 3.3 keeps District Heat report-only and Stage 3.4 is active for a non-punitive warning contract before any player-facing heat behavior. The simulator may expose `districtHeatProjection` and `districtHeatPromotionDecision` in full debug JSON, but current `SaveData`, browser storage, import/export save JSON, and cloud `rawSave` payloads must not persist `districtHeat`, `districtHeatProjection`, `districtHeatPromotionDecision`, `projectedHeat`, `heatBand`, or any related live heat state. Because no live or persisted heat state exists, Stage 3.4 does not bump `SAVE_DATA_VERSION` beyond `13` by default and does not add a migration. A future persisted District Heat implementation must open a dedicated save-version slice before adding durable heat fields.
