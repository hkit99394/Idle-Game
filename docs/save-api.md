...
Stage 3.1 District Heat remains report-only. The simulator may expose region-level `districtHeatProjection` in full debug JSON, but current `SaveData`, browser storage, import/export save JSON, and cloud `rawSave` payloads must not persist `districtHeat`, `districtHeatProjection`, `projectedHeat`, `heatBand`, or any related live heat state. Because no live or persisted heat state exists, Stage 3.1 does not bump `SAVE_DATA_VERSION` beyond `13` and does not add a migration. A future persisted District Heat implementation must open a dedicated save-version slice before adding durable heat fields.
