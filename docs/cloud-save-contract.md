...
Stage 3.3 keeps District Heat report-only and Stage 3.4 is active for a non-punitive warning contract before any player-facing heat behavior. Slice 100.3 keeps the warning cloud-free: cloud envelopes must not add heat, district-attention, warning, acknowledgement, or conflict metadata, and `rawSave` must not persist `districtHeat`, `districtHeatProjection`, `districtHeatPromotionDecision`, `projectedHeat`, `heatBand`, `districtAttention`, or derived live heat state. A future persisted District Heat mechanic requires a dedicated save-version and cloud-contract slice before cloud stores accept heat fields.
