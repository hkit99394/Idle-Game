...
There is no browser automation dependency in the repo at the start of Stage 1.9. Epic 59 adds source-level responsive smoke coverage in `tests/web/responsivePanelSmoke.test.ts`, which checks the narrow viewport CSS contracts and the panel selectors for stage routes, battle log rows, roster/formation, equipment, counterplay, and save tools.
