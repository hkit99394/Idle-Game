...
Stage 3.1 closed District Heat as an author-facing projection only. Slice 98.6 keeps that posture: `npm run simulate` and full debug JSON can show projected heat plus the `report_only` promotion decision, but the stable compact JSON/CSV exports, tactic exports, live rewards, save state, cloud payloads, and web UI remain heat-free. The next pacing step should be parity cleanup or a separate non-punitive warning contract, not a live heat reward/risk change.
