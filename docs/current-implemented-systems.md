...
This is the quick onboarding snapshot for the current Path of Neon implementation after Stage 3.1 closure. The project was previously called Path of Jianghu; older planning docs are still useful for intent, but this page is the short current-state reference.
...
- The retheme has its first deeper mechanic beyond copy replacement: Stage 3.0 implemented Intrusion through the [Cognitive Intrusion Prototype Contract](cognitive-intrusion-prototype-contract.md) and archived the closure record at [Archived Stage 3.0 Backlog](archive/stage-3.0-backlog.md). Stage 3.1 closed District Heat contract and pacing hardening at [Archived Stage 3.1 Backlog](archive/stage-3.1-backlog.md), starting from [District Heat Contract](district-heat-contract.md). Stage 3.2 is active at [Stage 3.2 Backlog](stage-3.2-backlog.md) for offline parity, Redline tuning closure, and release hardening. Slice 98.6 chose to keep District Heat report-only in simulator/default report/full debug JSON; live heat UI, save state, compact export fields, tactic export fields, reward modifiers, augment loadouts, network operations, countermeasure economy, and AI raid events remain deferred until their contracts and verification gates are ready.
...
- Stage 3.2 Pacing Parity And Heat Readiness is active at [Stage 3.2 Backlog](stage-3.2-backlog.md). It owns Epic 98: offline parity reporting, offline formula decision, Redline live-heat blocker triage/tuning, and a District Heat promotion decision before any live heat mechanic. Slice 98.3 keeps the live offline formula on the fixed 10s estimate while report-only parity classifies current recommended farms as `inversion`, `watch`, or `acceptable`. Slice 98.5 resolved the default Redline live-heat blockers. Slice 98.6 chose the `report_only` District Heat posture, so Stage 3.3 should resolve offline parity and visible Black Iron debt or open a separate non-punitive warning contract before any player-facing heat UI or reward behavior.
