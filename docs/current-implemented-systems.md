...
- The active Stage 2.6 planning backlog is [Stage 2.6 Backlog](stage-2.6-backlog.md), focused on static content id migration for enemies, heroes, skills, styles, equipment, assignments, medicines, statuses, and tactics. Slices 91.1 through 91.5 are complete: [Stage 2.6 Content Id Preflight](stage-2.6-content-id-preflight.md) owns the migrate/keep/defer matrix, `core/compatibility/contentIdAliases.ts` owns the alias data, save version `12` normalizes save-stored content aliases, and static hostile/status/initiate/protocol/style ids now use their canonical Path of Neon ids.
