...
- The retheme has its first deeper mechanic beyond copy replacement: Stage 3.0 implemented Intrusion through the [Cognitive Intrusion Prototype Contract](cognitive-intrusion-prototype-contract.md), Stage 3.1 closed District Heat contract and pacing hardening at [Archived Stage 3.1 Backlog](archive/stage-3.1-backlog.md), Stage 3.2 closed pacing parity and heat readiness at [Archived Stage 3.2 Backlog](archive/stage-3.2-backlog.md), Stage 3.3 closed offline parity, Black Iron visible-debt cleanup, and the heat promotion recheck at [Archived Stage 3.3 Backlog](archive/stage-3.3-backlog.md), and Stage 3.4 completed the warning contract at [Archived Stage 3.4 Backlog](archive/stage-3.4-backlog.md).
- District Heat remains report-only in simulator/default report/full debug JSON. Stage 3.4 selected stronger report-only guard rails instead of a runtime warning prototype: approved future warning copy uses neutral `district attention` wording, but live web source still blocks that copy until [Stage 3.5 Backlog](stage-3.5-backlog.md) deliberately amends the contract for a tiny route-card prototype. Live heat UI beyond that route-card warning, save state, compact export fields, tactic export fields, reward modifiers, augment loadouts, network operations, countermeasure economy, and AI raid events remain deferred until their contracts and verification gates are ready.
...
- Stage 3.5 District Attention Route-Card Prototype is active at [Stage 3.5 Backlog](stage-3.5-backlog.md). It owns Epic 101: contract reopen, owner-file allowlist, non-persistent route-card warning data, compact UI, save/cloud/export/reward boundary proof, browser smoke, and release hardening. `docs/stage-3.5-backlog.md` should remain active until Stage 3.5 is complete and archived.
