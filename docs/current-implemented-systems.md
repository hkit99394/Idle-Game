...
This is the quick onboarding snapshot for the current Path of Neon implementation after Stage 3.6 warning-only closure. The project was previously called Path of Jianghu; older planning docs are still useful for intent, but this page is the short current-state reference. For short, middle, and long-term milestone direction, use [Path Of Neon Roadmap](path-of-neon-roadmap.md).
...
- Path of Neon is the active product/theme direction. The milestone roadmap is documented in [Path Of Neon Roadmap](path-of-neon-roadmap.md), and the retheme plan is documented in [Path Of Neon Theme Bible](path-of-neon-theme-bible.md), [Path Of Neon Terminology Map](path-of-neon-terminology-map.md), and [Path Of Neon Retheme Migration Plan](retheme-migration-plan.md).
- The retheme has its first deeper mechanic beyond copy replacement: Stage 3.0 implemented Intrusion through the [Cognitive Intrusion Prototype Contract](cognitive-intrusion-prototype-contract.md), Stage 3.1 closed District Heat contract and pacing hardening at [Archived Stage 3.1 Backlog](archive/stage-3.1-backlog.md), Stage 3.2 closed pacing parity and heat readiness at [Archived Stage 3.2 Backlog](archive/stage-3.2-backlog.md), Stage 3.3 closed offline parity, Black Iron visible-debt cleanup, and the heat promotion recheck at [Archived Stage 3.3 Backlog](archive/stage-3.3-backlog.md), Stage 3.4 completed the warning contract at [Archived Stage 3.4 Backlog](archive/stage-3.4-backlog.md), Stage 3.5 shipped the warning-only route-card prototype at [Archived Stage 3.5 Backlog](archive/stage-3.5-backlog.md), and Stage 3.6 completed warning-only District Heat closure at [Archived Stage 3.6 Backlog](archive/stage-3.6-backlog.md).
- District Heat is warning-only for Stage 3.6: simulator/default report/full debug JSON keep report-only projection evidence, the promotion report can say `warning_only` when gates pass, and live UI has one neutral `district attention` route-card note for the selected offline farm route while it remains farmable. Live heat UI beyond that route-card warning, save state, compact export fields, tactic export fields, reward modifiers, augment loadouts, network operations, countermeasure economy, and AI raid events remain deferred until their contracts and verification gates are ready.
...
- Completed backlogs through Stage 3.4 live in `docs/archive`.
...
- Stage 3.3 Offline Parity And Visible Debt Cleanup is closed and archived at [Archived Stage 3.3 Backlog](archive/stage-3.3-backlog.md). It completed Epic 99: parity/debt evidence, target-derived offline formula implementation, `black_iron_foundry_4` cleanup, District Heat promotion recheck, and release hardening. Slice 99.3 moved current recommended farms to `acceptable` parity with no save/export/cloud/live-heat schema changes. Slice 99.4 resolved the remaining Black Iron visible debt with a stage-local sentry tune. Slice 99.5 reran promotion, kept current heat runtime report-only, and handed Stage 3.4 to a non-punitive warning contract before any live reward or route-risk behavior. `docs/stage-3.3-backlog.md` should not exist as an active backlog unless Stage 3.3 is explicitly reopened.
- Stage 3.4 District Heat Warning Contract is closed and archived at [Archived Stage 3.4 Backlog](archive/stage-3.4-backlog.md). It completed Epic 100: warning preflight, neutral `district attention` copy scope, future route-card placement, no-persistence/no-export warning boundaries, stronger live-web source guard rails, and release hardening. Stage 3.5 has since completed the dedicated tiny route-card prototype that Stage 3.4 handed off. `docs/stage-3.4-backlog.md` should not exist as an active backlog unless Stage 3.4 is explicitly reopened.
- Stage 3.5 District Attention Route-Card Prototype is closed and archived at [Archived Stage 3.5 Backlog](archive/stage-3.5-backlog.md). It completed Epic 101: contract reopen, owner-file allowlist, non-persistent route-card warning data, compact UI, save/cloud/export/reward boundary proof, browser smoke, and release hardening. Stage 3.6 later chose warning-only District Heat for closure instead of a live-heat mechanic. `docs/stage-3.5-backlog.md` should not exist as an active backlog unless Stage 3.5 is explicitly reopened.
- Stage 3.6 District Heat Live Decision is closed and archived at [Archived Stage 3.6 Backlog](archive/stage-3.6-backlog.md). It completed Epic 102: handoff evidence, decision matrix, warning-only contract update, boundary proof, roadmap handoff, release hardening, and archive readiness. The next backlog target is Stage 4.0 Next Neon System Selection, not more District Heat live work unless a future contract reopens it. `docs/stage-3.6-backlog.md` should not exist as an active backlog unless Stage 3.6 is explicitly reopened.
