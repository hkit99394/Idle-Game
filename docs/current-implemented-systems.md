...
This is the quick onboarding snapshot for the current Path of Neon implementation after Stage 3.4 release hardening. The project was previously called Path of Jianghu; older planning docs are still useful for intent, but this page is the short current-state reference. For short, middle, and long-term milestone direction, use [Path Of Neon Roadmap](path-of-neon-roadmap.md).
...
- Stage 3.4 District Heat Warning Contract is closed and archived at [Archived Stage 3.4 Backlog](archive/stage-3.4-backlog.md). It completed Epic 100: warning preflight, neutral `district attention` copy scope, future route-card placement, no-persistence/no-export warning boundaries, stronger live-web source guard rails, and release hardening. Current District Heat runtime stays report-only. The next implementation step, if desired, is a dedicated tiny route-card prototype slice that amends the guard with an explicit allowlist and proves rewards, enemy pressure, offline gains, saves, cloud payloads, stable exports, and route risk remain unchanged. `docs/stage-3.4-backlog.md` should not exist as an active backlog unless Stage 3.4 is explicitly reopened.
- Stage 3.5 District Attention Route-Card Prototype is closed and archived at [Archived Stage 3.5 Backlog](archive/stage-3.5-backlog.md). It completed Epic 101: contract reopen, owner-file allowlist, non-persistent route-card warning data, compact UI, save/cloud/export/reward boundary proof, browser smoke, and release hardening. The next step is Stage 3.6, which should decide whether District Heat stays warning-only or advances through a dedicated live-heat contract. `docs/stage-3.5-backlog.md` should not exist as an active backlog unless Stage 3.5 is explicitly reopened.
