...
- District Heat remains report-only in simulator/default report/full debug JSON. Stage 3.4 selected stronger report-only guard rails instead of a runtime warning prototype: approved future warning copy uses neutral `district attention` wording, but live web source still blocks that copy until a later dedicated route-card prototype slice amends the contract. Live heat UI, save state, compact export fields, tactic export fields, reward modifiers, augment loadouts, network operations, countermeasure economy, and AI raid events remain deferred until their contracts and verification gates are ready.
