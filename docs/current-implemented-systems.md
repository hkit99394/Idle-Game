...
- Stage 2.9 cleanup and Cognitive Intrusion handoff is tracked at [Stage 2.9 Backlog](stage-2.9-backlog.md). Slice 2.9.1 kept temporary tactic comparison legacy damage columns for one more review cycle, Slice 2.9.2 kept engine-level `outerDamage` / `innerDamage` replay/accounting payload fields stable while public/report surfaces continue to use current Kinetic/Cognitive or Body Integrity/Context Stability wording, Slice 2.9.3 kept static taxonomy ids/buckets stable while updating safe display/docs language, and Slice 2.9.4 refreshed the Cognitive Intrusion contract around one new `cognitive_intrusion` status and one new `cognitiveDamageTakenMultiplier` status modifier with no save/export/event/taxonomy churn.
