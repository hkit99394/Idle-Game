...
- Stage 3.2 Pacing Parity And Heat Readiness is closed and archived at [Archived Stage 3.2 Backlog](archive/stage-3.2-backlog.md). It completed Epic 98: offline parity reporting, offline formula decision, Redline live-heat blocker triage/tuning, District Heat promotion decision, and release hardening. Slice 98.3 keeps the live offline formula on the fixed 10s estimate while report-only parity classifies current recommended farms as `inversion`, `watch`, or `acceptable`. Slice 98.5 resolved the default Redline live-heat blockers. Slice 98.6 chose the `report_only` District Heat posture, so Stage 3.3 should resolve offline parity and visible Black Iron debt or open a separate non-punitive warning contract before any player-facing heat UI or reward behavior. `docs/stage-3.2-backlog.md` should not exist as an active backlog unless Stage 3.2 is explicitly reopened.
- Stage 3.3 Offline Parity And Visible Debt Cleanup is active at [Stage 3.3 Backlog](stage-3.3-backlog.md). It owns Epic 99: re-baseline parity/debt evidence, decide and implement the smallest safe offline formula path or guard rail, clean up `black_iron_foundry_4`, and recheck District Heat promotion before any player-facing heat behavior.
