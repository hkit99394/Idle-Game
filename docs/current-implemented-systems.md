...
This is the quick onboarding snapshot for the current Path of Neon implementation after Stage 3.0 closure. The project was previously called Path of Jianghu; older planning docs are still useful for intent, but this page is the short current-state reference.
...
- The retheme has its first deeper mechanic beyond copy replacement: Stage 3.0 implemented Intrusion through the [Cognitive Intrusion Prototype Contract](cognitive-intrusion-prototype-contract.md) and archived the closure record at [Archived Stage 3.0 Backlog](archive/stage-3.0-backlog.md). [Stage 3.1 Backlog](stage-3.1-backlog.md) is active for District Heat contract and pacing hardening, starting from [District Heat Contract](district-heat-contract.md); live District Heat, augment loadouts, network operations, countermeasure economy, and AI raid events remain deferred until their contracts and verification gates are ready.
...
- Completed backlogs through Stage 3.0 live in `docs/archive`.
...
- Stage 3.1 District Heat Contract And Pacing Hardening is active at [Stage 3.1 Backlog](stage-3.1-backlog.md). It owns Epic 97: contract-first District Heat preparation, offline/pacing proof, known debt classification, report-only heat projection, and save/UI/export boundary decisions before any live heat mechanic.
