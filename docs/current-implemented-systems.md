...
- Stage 2.9 cleanup and Cognitive Intrusion handoff is tracked at [Stage 2.9 Backlog](stage-2.9-backlog.md). Slice 2.9.1 kept temporary tactic comparison legacy damage columns for one more review cycle, and Slice 2.9.2 kept engine-level `outerDamage` / `innerDamage` replay/accounting payload fields stable while public/report surfaces continue to use current Kinetic/Cognitive or Body Integrity/Context Stability wording. Later slices own deferred static taxonomy cleanup and the Cognitive Intrusion prototype contract refresh.
