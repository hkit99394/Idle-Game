...
- Stage 3.2 Pacing Parity And Heat Readiness is active at [Stage 3.2 Backlog](stage-3.2-backlog.md). It owns Epic 98: offline parity reporting, offline formula decision, Redline live-heat blocker triage/tuning, and a District Heat promotion decision before any live heat mechanic. Slice 98.3 keeps the live offline formula on the fixed 10s estimate while report-only parity classifies current recommended farms as `inversion`, `watch`, or `acceptable`. Slice 98.4 triaged Redline live-heat blockers and assigned 98.5 target/data/status actions; live District Heat remains blocked until those gates close.
