...
This is the quick onboarding snapshot for the current Path of Neon implementation after Stage 3.3 closure and Stage 3.4 backlog preparation. The project was previously called Path of Jianghu; older planning docs are still useful for intent, but this page is the short current-state reference.
...
- The retheme has its first deeper mechanic beyond copy replacement: Stage 3.0 implemented Intrusion through the [Cognitive Intrusion Prototype Contract](cognitive-intrusion-prototype-contract.md), Stage 3.1 closed District Heat contract and pacing hardening at [Archived Stage 3.1 Backlog](archive/stage-3.1-backlog.md), Stage 3.2 closed pacing parity and heat readiness at [Archived Stage 3.2 Backlog](archive/stage-3.2-backlog.md), Stage 3.3 closed offline parity, Black Iron visible-debt cleanup, and the heat promotion recheck at [Archived Stage 3.3 Backlog](archive/stage-3.3-backlog.md), and Stage 3.4 is active at [Stage 3.4 Backlog](stage-3.4-backlog.md).
- District Heat remains report-only in simulator/default report/full debug JSON. Stage 3.4 owns the non-punitive warning contract as the next heat milestone, but live heat UI, save state, compact export fields, tactic export fields, reward modifiers, augment loadouts, network operations, countermeasure economy, and AI raid events remain deferred until their contracts and verification gates are ready.
...
- Stage 3.4 District Heat Warning Contract is active at [Stage 3.4 Backlog](stage-3.4-backlog.md). It owns Epic 100: copy scope, UI placement, save posture, export posture, report relationship, and guard rails for a future non-punitive warning. Current District Heat runtime stays report-only until Stage 3.4 explicitly chooses the next implementation step.
