...
This document is the short, middle, and long-term milestone map for Path of Neon after Stage 3.6 warning-only closure. It summarizes direction across the active docs; detailed contracts and verification requirements still live in the linked source documents.
...
Path of Neon has completed the display-safe retheme, the first neon-native mechanic prototype, the District Heat warning-only route-card prototype, and the Stage 3.6 District Heat posture decision:
...
- Stage 3.6 selected warning-only District Heat for closure, re-proved save/cloud/export/reward/offline boundaries, and is archived at [Archived Stage 3.6 Backlog](archive/stage-3.6-backlog.md).

The current runtime has one warning-only District Attention route-card note for the selected offline farm target while it remains farmable. The author-facing promotion report may say `warning_only` when gates pass, but District Heat still has no save field, cloud field, compact export field, tactic export field, reward modifier, route-risk modifier, enemy-pressure modifier, global heat meter, acknowledgement state, timer, or number.

## Short Term

### Stage 4.0: Next Neon System Selection

Stage 3.6 is archived. The next backlog target is Stage 4.0, which should choose the first post-District-Heat neon-native system without reopening District Heat unless a new contract explicitly asks for it.

Goal:

- Pick one next neon-native system and write its first milestone contract before implementation.
- Keep District Heat warning-only while Stage 4.0 chooses between build depth, idle depth, countermeasure depth, online identity, or hostile Intrusion expansion.

District Heat carry-forward surface:
...
Stage 4.0 carry-forward guard rails:
...
- Do not add a global meter, top-bar badge, modal, onboarding panel, district header, offline summary total, warning acknowledgement, save diagnostic, severity band, timer, number, heat meter, or durable heat field without a dedicated later District Heat contract.
...
Browser smoke is required if Stage 4.0 changes visible UI or adds a new player-facing District Heat surface.

Stage 3.6 decision outcome:

| Option | Meaning | Default risk |
| --- | --- | --- |
| Warning-only | Keep the route-card note and no gameplay changes. | Selected for Stage 3.6. |
| Report-only tooling | Improve simulator evidence without changing player behavior. | Deferred unless a future authoring stage needs it. |
| Offline recommendation scoring | Let heat influence author-facing or future route recommendation logic without changing rewards. | Deferred to a later contract. |
| Tiny live effect | Add one bounded effect after a dedicated save/export/UI contract. | Rejected for Stage 3.6. |
| Persisted heat | Store district heat in saves and cloud payloads. | Rejected until a future save/cloud milestone. |

District Heat carry-forward posture:

- Keep District Heat **warning-only** through Stage 3.6 closure.
- Keep Stage 3.6 archived unless a future contract explicitly reopens District Heat.
- Do not add reward modifiers, enemy-pressure modifiers, route-risk modifiers, or persisted heat without a later dedicated save/export/UI contract.

## Middle Term

### Stage 4.x: First Selected Neon System Prototype

After Stage 4.0 selects a candidate, implement the smallest useful prototype for that selected system.
...
Recommended next candidate after District Heat: **Augment Loadouts** if the goal is build depth, or **Network Operations** if the goal is idle/assignment depth. Keep Countermeasure Economy, AI Raid Event, Hostile Cognitive Intrusion, and persisted District Heat as separate later milestones unless Stage 4.0 deliberately selects one.
...
1. Archive warning-only District Heat and keep any live-heat work behind a later dedicated contract.
