...
[Stage 3.3 Backlog](stage-3.3-backlog.md) owns that next cleanup path. Slice 99.2 chooses a target-derived offline reward estimate for the next implementation slice, using existing route clear-time target bands instead of adding stage-authored estimates or making live save load depend on simulator output. District Heat should recheck promotion only after that parity work and `black_iron_foundry_4` have current evidence.
