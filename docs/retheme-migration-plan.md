...
- Static data `id` values and all save/static `*Id` reference fields stay stable until their dedicated internal-id migration slice. Stage 2.5 migrated region and route id values, and Stage 2.6 migrated static content id values; persisted save fields remain owned by later stages.
